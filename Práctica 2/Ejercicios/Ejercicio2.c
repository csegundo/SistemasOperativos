...
int main(int argc, char *argv[]) {
	...
	int parteOpcional = 0;
	if(strcmp(argv[1], "-L") == 0) //Si pasamos como parametro -L, necesitaremos llamar a lstat() en lugar de stat()
		parteOpcional = 1;
	for (i=1 + parteOpcional; i<argc; i++)
		status(argv[i], parteOpcional);

	exit(0);
}

int status(char *filename, int parteOpcional) {
	struct stat buf;
	struct passwd *pw;
	struct group *gr;
	int i;

	if(parteOpcional == 1){ //Esto quiere decir que se ha pasado -L como parametro
		if (lstat(filename, &buf) == -1) //lstat nos muestra la informacion del enlace simbolico.
