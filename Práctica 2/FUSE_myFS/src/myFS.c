...
int readInodes(MyFileSystem* myFileSystem)
{
    ...
    NodeStruct * nsParcial;
    for(int i = 0; i < MAX_NODES; ++i){
        nsParcial = malloc(sizeof(NodeStruct));
        if(lseek(myFileSystem->fdVirtualDisk, findNodeByPos(i), SEEK_SET) == (off_t) - 1) {
            perror("Failed lseek in readInodes");
            return -1;
        }
        else if(read(myFileSystem->fdVirtualDisk, nsParcial, sizeof(NodeStruct)) == -1) {
            perror("Failed read in readInodes");
            return -1;
        }
        else {
            if(nsParcial->freeNode == 0){ //i-nodo ocupado
                myFileSystem->nodes[i] = nsParcial;
                //numBloquesOcupados += nsParcial->numBlocks;
